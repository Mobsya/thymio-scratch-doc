...
\trickbox{To set the color, click the little square and
then move the mouse pointer and click an area on a sprite or the
...
\scrblk[-6]{get-pointer-direction-block} is shown in \cref{fig.get-dir}. We
first declare the variable \scrblk[-3]{save-dir} which is used internally
by this block, and the variable \scrblk[-3]{direction-to-pointer} which is
...
\scrblk[-3]{save-dir}, we get the difference between the two direction. The
